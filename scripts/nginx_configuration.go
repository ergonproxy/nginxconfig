...
func main() {
	b, err := format.Source(dumpKeys(Core{}, Events{}, SharedHTTP{}, HTTP{}, Server{}))
	if err != nil {
		log.Fatal(err)
	}
	err = ioutil.WriteFile("ngx_ctx_keys.go", b, 0600)
	if err != nil {
		log.Fatal(err)
	}
}

func dumpKeys(ob ...interface{}) []byte {
	var fields []reflect.StructField
	for _, v := range ob {
		o := reflect.TypeOf(v)
		for i := 0; i < o.NumField(); i++ {
			fields = append(fields, o.Field(i))
		}
	}
	...
	buf.WriteString(`// DO NOT EDIT this file was auto generated
	package main
	`)
	...
	for _, f := range fields {
		...
		fmt.Fprintf(&buf, "%sKey struct{} //%s\n", toName(f.Name), f.Tag.Get("nginx"))
		m[f.Name] = true
	}
	buf.WriteString(")")
	return buf.Bytes()
...
func toName(n string) string {
	return string(unicode.ToLower(rune(n[0]))) + n[1:]
}

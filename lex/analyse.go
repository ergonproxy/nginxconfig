...
func analyze(stmt *config.Directive, tok int, strict, checkCtx, checkArgs bool, ctx []string) error {
	...
	if checkCtx {
		...
		if !pass {
			return fmt.Errorf("nginx: %s directive %q not allowed ", stmt.Start, stmt.Name)
		}
	}
	if !checkArgs {
		return nil
	...
	for _, mask := range reverse(masks) {
		if (mask&NGX_CONF_BLOCK != 0) && tok != LBrace {
			return fmt.Errorf("nginx: %s directive %q has no opening { ", stmt.Start, stmt.Name)
		}
		if !(mask&NGX_CONF_BLOCK != 0) && tok != SColon {
			return fmt.Errorf("nginx: %s directive %q is not terminated by ; ", stmt.Start, stmt.Name)
		}
		if ((mask>>uint(n))&1 != 0 && n <= 7) ||
			(mask&NGX_CONF_FLAG != 0 && n == 1 && validFlag(stmt.Params[0].Text)) ||
			(mask&NGX_CONF_ANY != 0 && n >= 0) ||
			(mask&NGX_CONF_1MORE != 0 && n >= 1) ||
			(mask&NGX_CONF_2MORE != 0 && n >= 2) {
			return nil
		} else if mask&NGX_CONF_FLAG != 0 && n == 1 && !validFlag(stmt.Params[0].Text) {
			p := stmt.Params[0]
			return fmt.Errorf("nginx:%s invalid value %q in directive %q  it must be \"on\" or \"off\" ",
				p.Start, p.Text, stmt.Name)
		} else {
			return fmt.Errorf("nginx: %s directive %q invalid number of arguments ; ", stmt.Start, stmt.Name)
		}
	}
